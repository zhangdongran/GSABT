...
def main():
    ...
    log_file='parameter/GSABT_{}_{: 04d}-{: 02d}-{: 02d}-{: 02d}-{: 02d}.txt'.\
    ...
    train_loader, valid_loader, test_loader, f_mean, f_std, s_mean, s_std, t_mean, t_std,\
    nodes, f_nodes, s_nodes, t_nodes, matrix, log = utils.tbt(args, log)
    ...
    from model.T_MPBTCN import Network

    model = Network(matrix, f_nodes, s_nodes, t_nodes, args.blocks, args.feature, args.input_dim, args.hidden_dim, args.output_dim, args.dropout).to(device)
    ...
    f_output = f_output * f_std +f_mean
    f_target = f_target * f_std + f_mean

    s_output = output[:, :, f_nodes:-t_nodes, :]
    s_target = target[:, :, f_nodes:-t_nodes, :]
    s_output = s_output * s_std + s_mean
    s_target = s_target * s_std + s_mean

    t_output = output[:, :, -t_nodes:, :]
    t_target = target[:, :, -t_nodes:, :]
    t_output = t_output * t_std + t_mean
    t_target = t_target * t_std + t_mean
    ...
    log_string(log, '第三模态')
    error(t_output, t_target, args, log)
