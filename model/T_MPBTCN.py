...
class Spatial(nn.Module):
    def __init__(self, feature, input_dim):
        ...
        self.graph_out = nn.Linear(dmodel, dmodel)
        self.act = nn.ReLU()
    ...
    def forward(self,x, graph): # (B, T, N, D)
        ...
        if graph is None:
            ...
            graph_out = torch.matmul(graph_mask, self.linear2(graph_out))  # (B, N, T*D)
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, groups, dilation, padding, dropout):
        ...
        self.conv = nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups)
        self.chomp = Chomp(padding)
        self.relu = nn.ReLU()
...
class ST_Block(nn.Module):
    def __init__(self, adj_data, f_nodes, s_nodes, t_nodes, feature, input_dim, hidden_dim, output_dim, dropout):
        ...
        self.nodes = f_nodes + s_nodes + t_nodes

        self.spatial = Spatial(feature, input_dim)

        self.btcn_f = BTCN(self.f_nodes * feature, groups=1, dropout=dropout)
        self.btcn_s = BTCN(self.s_nodes * feature, groups=1, dropout=dropout)
        self.btcn_t = BTCN(self.t_nodes * feature, groups=1, dropout=dropout)

        self.btcn = BTCN(self.nodes * feature, groups=1, dropout=dropout)
    ...
    def forward(self, x):  # (B, T, N, D)
        ...
        t_f = t[:, :, :self.f_nodes, :]
        t_s = t[:, :, self.f_nodes:-self.t_nodes, :]
        t_t = t[:, :, -self.t_nodes:, :]
        t_f = self.btcn_f(t_f)
        t_s = self.btcn_s(t_s)
        t_t = self.btcn_t(t_t)

        t2 = torch.cat([t_f, t_s, t_t], dim=2)
...
class Network(nn.Module):
    def __init__(self, adj_data, f_nodes, s_nodes, t_nodes, blocks, feature, input_dim, hidden_dim, output_dim, dropout):
        ...
        for i in range(self.blocks):
            st_blocks.append(ST_Block(adj_data, f_nodes, s_nodes, t_nodes, feature, input_dim, hidden_dim, output_dim, dropout))
