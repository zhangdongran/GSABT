...
class ST_Block(nn.Module):
    def __init__(self, adj_data,  nodes, feature, input_dim, hidden_dim, output_dim, dropout):
...
class Network(nn.Module):
    def __init__(self, adj_data, nodes, blocks, feature, input_dim, hidden_dim, output_dim, dropout):
        super(Network, self).__init__()

        self.blocks = blocks
        st_blocks = []
        for i in range(self.blocks):
            st_blocks.append(ST_Block(adj_data, nodes,  feature, input_dim, hidden_dim, output_dim, dropout))
        self.st = nn.ModuleList(st_blocks)
        self.mlp = MLP(input_dim * (blocks + 1), hidden_dim, output_dim, dropout=dropout)

        self.betcn = BTCN(nodes * feature, nodes * feature, dropout)
